...
import { interstitialAd, rewardedAd } from './hooks/ads';
...
const COLORS = {
    ...
    numpadBtn: '#FDFBF7', numpadComplete: '#C8DDD7', noteText: '#7A9E95',
};
...
function generateSudoku(diff: Difficulty) {
    ...
    let board: number[][] = [];
    for (const b of bo) { const rows = shuffle([b * 3, b * 3 + 1, b * 3 + 2]); for (const r of rows) board.push([...BASE[r]]); }
    board = transpose(board);
    const co = shuffle([0, 1, 2]); let final: number[][] = [];
    for (const b of co) { const cols = shuffle([b * 3, b * 3 + 1, b * 3 + 2]); for (const c of cols) final.push(board[c]); }
...
function useTimer(running: boolean) {
    ...
    useEffect(() => { if (!running) return; const id = setInterval(() => setS(x => x + 1), 1000); return () => clearInterval(id); }, [running]);
    const reset = () => setS(0);
...
export default function SoftSudoku() {
    ...
    useEffect(() => {
        ...
        const u2 = interstitialAd.addAdEventListener(AdEventType.CLOSED, () => { setAdLoaded(false); interstitialAd.load(); });
        const u3 = rewardedAd.addAdEventListener(RewardedAdEventType.LOADED, () => setRewardedLoaded(true));
        const u4 = rewardedAd.addAdEventListener(AdEventType.CLOSED, () => { setRewardedLoaded(false); rewardedAd.load(); });
        interstitialAd.load();
        rewardedAd.load();
        return () => { u1(); u2(); u3(); u4(); };
    }, []);
    ...
    // Rewarded ad → reveal one random empty cell
    const handleHint = () => {
        if (!rewardedLoaded || !gameState) return;
        const u = rewardedAd.addAdEventListener(RewardedAdEventType.EARNED_REWARD, () => {
            u();
    ...
    const getCellBg = (r: number, c: number) => {
        ...
        const val = board[r][c], isFixed = gameState.puzzle[r][c] !== 0;
        ...
        if (selected) { const { r: sr, c: sc } = selected; const sameBox = Math.floor(r / 3) === Math.floor(sr / 3) && Math.floor(c / 3) === Math.floor(sc / 3); if (r === sr || c === sc || sameBox) return COLORS.sameRowCol; }
    ...
    return (
        <ScrollView style={[s.container, { backgroundColor: COLORS.background }]} contentContainerStyle={[s.content, { paddingHorizontal: 24 }]} bounces={false}>
            <View style={s.header}>
                <Pressable style={s.ib} onPress={handleBack}><ArrowLeft size={22} color={COLORS.text} /></Pressable>
                <Text style={s.title}>Soft Sudoku</Text>
                <Pressable style={s.ib} onPress={() => startGame()}><RotateCcw size={22} color={COLORS.text} /></Pressable>
            </View>
            <View style={s.metaRow}>
                <View style={s.diffRow}>
                    {(['easy', 'medium', 'hard'] as Difficulty[]).map(d => (
                        <Pressable key={d} style={[s.diffBtn, difficulty === d && s.diffBtnA]} onPress={() => { setDifficulty(d); startGame(d); }}>
                            <Text style={[s.diffTxt, difficulty === d && { color: '#FFF' }]}>{d.charAt(0).toUpperCase() + d.slice(1)}</Text>
                        </Pressable>
                    ))}
                </View>
                <View style={s.timerRow}><Clock size={12} color={COLORS.textLight} /><Text style={s.timerTxt}>{timerFmt}</Text></View>
            </View>
            <View style={s.statsRow}>
                <Text style={s.statLbl}>Mistakes</Text>
                <View style={s.dots}>{[0, 1, 2].map(i => <View key={i} style={[s.dot, mistakes > i && s.dotFilled]} />)}</View>
            </View>
            <View style={[s.gridWrap, { width: BOARD_SIZE + 4 }]}>
                <View style={[s.grid, { width: BOARD_SIZE }]}>
                    {board.map((row, r) => (
                        <View key={r} style={[s.row, r === 2 || r === 5 ? s.rowThick : null]}>
                            {row.map((cell, c) => {
                                const cNotes = notes[r][c]; const isFixed = gameState.puzzle[r][c] !== 0;
                                const isError = !isFixed && cell !== 0 && cell !== gameState.solution[r][c];
                                return (
                                    <Pressable key={c} style={[s.cell, { width: CELL_SIZE, height: CELL_SIZE, backgroundColor: getCellBg(r, c) }, c === 2 || c === 5 ? s.colThick : null]} onPress={() => setSelected({ r, c })}>
                                        {cell !== 0
                                            ? <Text style={[s.cellTxt, { fontSize: CELL_SIZE * 0.46, color: isError ? COLORS.errorText : isFixed ? COLORS.text : COLORS.accent, fontWeight: isFixed ? '700' : '500' }]}>{cell}</Text>
                                            : cNotes.size > 0 && <View style={s.notesGrid}>{[1, 2, 3, 4, 5, 6, 7, 8, 9].map(n => <Text key={n} style={[s.noteTxt, { fontSize: CELL_SIZE * 0.22, width: CELL_SIZE / 3 }]}>{cNotes.has(n) ? n : ' '}</Text>)}</View>
                                        }
                                    </Pressable>
                                );
                            })}
            ...
            {isWon && (
                <Animated.View style={[s.winBanner, { opacity: winAnim, transform: [{ scale: winAnim }] }]}>
                    <Text style={s.winTxt}>✦ Harmony Found ✦</Text>
                    <Text style={s.winSub}>{timerFmt} · {mistakes} mistake{mistakes !== 1 ? 's' : ''}</Text>
                </Animated.View>
            )}
            <View style={s.actionBar}>
                <Pressable style={s.actionBtn} onPress={() => { if (history.length) { const p = history[history.length - 1]; setBoard(p.board); setNotes(p.notes); setHistory(h => h.slice(0, -1)); } }}>
                    <Undo2 size={18} color={history.length > 0 ? COLORS.accent : COLORS.textLight} />
                    <Text style={[s.actionLbl, { color: history.length > 0 ? COLORS.accent : COLORS.textLight }]}>Undo</Text>
                </Pressable>
                <Pressable style={s.actionBtn} onPress={handleErase}>
                    <Text style={[s.eraseTxt, { color: COLORS.accent }]}>⌫</Text>
                    <Text style={[s.actionLbl, { color: COLORS.accent }]}>Erase</Text>
                </Pressable>
                <Pressable style={[s.actionBtn, pencilMode && { backgroundColor: COLORS.accent }]} onPress={() => setPencilMode(p => !p)}>
                    <Pencil size={18} color={pencilMode ? '#FFF' : COLORS.accent} />
                    <Text style={[s.actionLbl, { color: pencilMode ? '#FFF' : COLORS.accent }]}>Notes</Text>
                </Pressable>
                {/* Rewarded ad hint button */}
                <Pressable style={[s.actionBtn, !rewardedLoaded && { opacity: 0.4 }]} onPress={handleHint} disabled={!rewardedLoaded}>
                    <Text style={s.hintIcon}>💡</Text>
                    <Text style={[s.actionLbl, { color: '#D4A843' }]}>Hint</Text>
                </Pressable>
            </View>
            <View style={s.numpad}>
                {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => {
                    ...
                    return (
                        <Pressable key={num} style={[s.numBtn, { width: NUMPAD_BTN, height: NUMPAD_BTN * 1.15 }, done && s.numBtnDone]} onPress={() => handleNumberPress(num)} disabled={done}>
                            <Text style={[s.numBtnTxt, { fontSize: NUMPAD_BTN * 0.45, color: done ? COLORS.textLight : COLORS.accent }]}>{num}</Text>
                            {!done && <Text style={s.numCount}>{9 - numCounts[num - 1]}</Text>}
                        </Pressable>
                    );
                })}
            </View>
...
const s = StyleSheet.create({
    container: { flex: 1 }, content: { paddingTop: 60, paddingBottom: 40, alignItems: 'center' },
    header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', width: '100%', marginBottom: 14 },
    ib: { width: 40, height: 40, justifyContent: 'center', alignItems: 'center' },
    ...
    diffBtnA: { backgroundColor: '#6B9080' },
    ...
    actionBtn: { alignItems: 'center', justifyContent: 'center', backgroundColor: '#FDFBF7', borderRadius: 12, paddingVertical: 8, paddingHorizontal: 12, gap: 3, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3, elevation: 1 },
    actionLbl: { fontSize: 10, fontWeight: '600' },
    eraseTxt: { fontSize: 18, lineHeight: 20 },
    hintIcon: { fontSize: 16, lineHeight: 20 },
